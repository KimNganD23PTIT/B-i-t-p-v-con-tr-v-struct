...
// Hàm tìm kiếm các máy tính dựa trên điểm hiệu suất tối thiểu và giá tối đa
void searchComputers(const vector<Computer> &computers, float minPerformance, float maxPrice) {
    bool found = false;
    cout << "\n--- Kết quả tìm kiếm ---\n";
    for (size_t i = 0; i < computers.size(); ++i) {
    ...
    if (!found) {
        cout << "Không có máy tính nào đáp ứng yêu cầu về điểm hiệu suất và ngân sách.\n";
    }
}
...
int main() {
    ...
    do {
        ...
        cout << "4. Tìm kiếm cấu hình máy theo điểm hiệu suất và giá\n";
        cout << "0. Thoát\n";
        ...
        switch (choice) {
            ...
            case 4: {
                // Tìm kiếm cấu hình máy theo điểm hiệu suất và giá
                if (computers.empty()) {
                    cout << "Không có cấu hình máy nào để tìm kiếm.\n";
                    break;
                }
                
                float minPerformance, maxPrice;
                cout << "Nhập điểm hiệu suất tối thiểu: ";
                cin >> minPerformance;
                cout << "Nhập giá tối đa (USD): ";
                cin >> maxPrice;
                
                searchComputers(computers, minPerformance, maxPrice);
                break;
            }
            case 0:
